...
public class AniListRepository implements AnimeRepository {
    ...
    private String makeRequest(String body, KeyValuePair... variables) {
        ...
        for (KeyValuePair keyValuePair : variables)
            variablesObject.put(keyValuePair.key, keyValuePair.value);
    ...
    private <T> void includeVariable(List<KeyValuePair> variables, String key, Optional<T> variable) {
        variable.ifPresent(t -> variables.add(new KeyValuePair(key, t)));
    }

    @Override
    public Media searchMedia(String englishTitle) {
        String response = makeRequest("query($mediaTitle: String) { Media(search: $mediaTitle, type: ANIME) { " + ClassSerializer.formatClassFields(Media.class) + " } }",
                new KeyValuePair("mediaTitle", englishTitle));
        if (response == null)
            return null;
        Gson gson = new Gson();
        return gson.fromJson(new JSONObject(response).get("Media").toString(), Media.class);
    }

    @Override
    public MediaList searchMediaList(int mediaId) {
        String response = makeRequest("query($mediaId: Int) { MediaList(mediaId: $mediaId) { " + ClassSerializer.formatClassFields(MediaList.class) + " } }",
                new KeyValuePair("mediaId", mediaId));
        if (response == null)
            return null;
        Gson gson = new Gson();
        return gson.fromJson(new JSONObject(response).get("MediaList").toString(), MediaList.class);
    }

    @Override
    public void updateMediaList(int mediaId, Optional<Integer> progress, Optional<Float> score, Optional<MediaListStatus> status) {
        ...
        String response = makeRequest("mutation($mediaId: Int, $progress: Int, $score: Float, $status: MediaListStatus) { SaveMediaListEntry(mediaId: $mediaId, progress: $progress, score: $score, status: $status) { progress } }",
